...
impl From<ActiveEntity> for ActiveResponse {
...
impl TryFrom<ActiveResponse> for ActiveEntity {
    type Error = ApiError;

    fn try_from(active_response: ActiveResponse) -> Result<Self, Self::Error> {
        Ok(Self {
...
impl From<ActiveEntity> for ActiveRequest {
    fn from(active_entity: ActiveEntity) -> Self {
        Self {
            user_id: active_entity.user_id as i32,
            security_id: active_entity.security_id,
...
impl TryFrom<ActiveRequest> for ActiveEntity {
    type Error = ApiError;

    fn try_from(active_response: ActiveRequest) -> Result<Self, Self::Error> {
        Ok(Self {
            user_id: active_response.user_id as u32,
            security_id: active_response.security_id,
            bought_price: Price {
